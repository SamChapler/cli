...
menu:
  docs_0.11.0:
    identifier: my-group-replication
    name: Group Replication Guide
    parent: my-cluster
    weight: 15
...
- Read [redis clustering concept](/docs/guides/mysql/clustering/group-replication/overview.md) to learn about MySQL Group Replication.
...
```yaml
$ kubectl patch -n demo my my-galera -p '{"metadata":{"finalizers":[]}}' --type=merge; kdel -n demo my my-galera; kga pvc | grep my-galera | awk '{print $2 " -n " $1}' | xargs kubectl delete pvc; kga pv | grep my-galera | awk '{print $1}' | xargs kubectl delete pv; kcr -f dist/my-test.yaml
